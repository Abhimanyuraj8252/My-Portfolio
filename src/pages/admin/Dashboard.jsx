...
const Dashboard = () => {
    ...
    const fetchAllStats = async () => {
        ...
        try {
            ...
            const { data: testimonials } = await supabase.from('reviews').select('*');
            ...
            const contactStats = {
                ...
                unread: contacts?.filter(c => !c.is_read).length || 0,
                read: contacts?.filter(c => c.is_read).length || 0
            };

            const serviceBreakdown = {};
            contacts?.forEach(c => {
                const service = c.service || 'Other';
